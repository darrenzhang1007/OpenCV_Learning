...
int main(int artc, char** argv) {
	Mat src1 = imread("H:\\OpenCV_Learning\\pix_arith_opera\\LinuxLogo.jpg");
	Mat src2 = imread("H:\\OpenCV_Learning\\pix_arith_opera\\WindowsLogo.jpg");
	...
	for (int row = 0; row < height; row++) {
		for (int col = 0; col < width; col++) {
			...
			// uchar 是一个字节8位的；相加之后：[0-255] *2 -> [0-510] 会导致溢出。saturate_cast:大于255为255，负数的时候为0；
			result.at<Vec3b>(row, col)[0] = saturate_cast<uchar>(b1 + b2); // 不丢失精度的数据类型转换
